...
public class TranslateDeclarativeConstriant2DeclarativeFormula extends VisitReturn<Object> {
   ...
   */
  private TranslateDeclarativeConstriant2DeclarativeFormula (A4Reporter rep, A4Options opt, Iterable<Sig> sigs, Command cmd) throws Err {
   ...
   */
  private TranslateDeclarativeConstriant2DeclarativeFormula (int bitwidth, int unrolls, Map<Expr,Expression> a2k, Map<String,Expression> s2k) throws Err {
 ...
 private void makeFacts(Expr facts) throws Err {
  ...
  private static A4Solution execute_greedyCommand(A4Reporter rep, Iterable<Sig> sigs, Command usercommand, A4Options opt) throws Exception {
      ...
      TranslateDeclarativeConstriant2DeclarativeFormula tr = null;
      try {
  ...
  public static List<Formula> translate(A4Reporter rep, Iterable<Sig> sigs, Command cmd, A4Options opt) throws Err{
    if (rep==null) rep = A4Reporter.NOP;
    TranslateDeclarativeConstriant2DeclarativeFormula tr = null;
    try {
        if (cmd.parent!=null || !cmd.getGrowableSigs().isEmpty()) throw new RuntimeErrorException(null, "Should not be reched");
        tr = new TranslateDeclarativeConstriant2DeclarativeFormula(rep, opt, sigs, cmd);
        tr.makeFacts(cmd.formula);
        return tr.frame.getKKFormulas();//.
  ...
  public static Object alloy2kodkod(A4Solution sol, Expr expr) throws Err {
      ...
      TranslateDeclarativeConstriant2DeclarativeFormula tr = new TranslateDeclarativeConstriant2DeclarativeFormula(sol.getBitwidth(), sol.unrolls(), sol.a2k(), sol.s2k());
      Object ans;
      try {
