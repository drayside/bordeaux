package edu.uw.ece.bordeaux.util.graph;
...
public class DijkstraSP {
	...
	public DijkstraSP(EdgeWeightedDigraph G, int s) {
		for (DirectedEdge e : G.edges()) {
			// if (e.weight() < 0)
			// throw new IllegalArgumentException("edge " + e + " has negative
			// weight");
		}
	...
	/**
	 * Returns the length of a shortest path from the source vertex <tt>s</tt> to
	 * vertex <tt>v</tt>.
	 * 
	 * @param v
	 *          the destination vertex
	 * @return the length of a shortest path from the source vertex <tt>s</tt> to
	 *         vertex <tt>v</tt>; <tt>Double.POSITIVE_INFINITY</tt> if no such
	 *         path
	 */
	 ...
	 * @param v
	 *          the destination vertex
	 * @return <tt>true</tt> if there is a path from the source vertex <tt>s</tt>
	 *         to vertex <tt>v</tt>, and <tt>false</tt> otherwise
	 */
