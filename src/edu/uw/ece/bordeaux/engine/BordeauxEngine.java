...
public final class BordeauxEngine {
	...
	private final static Logger logger = Logger.getLogger(BordeauxEngine.class.getName() + "--" + Thread.currentThread().getName());
	...
	public void getBorderInstances(File inputPath, String...predNames) {
		...
		File outputPath = new File(this.tmpPath.getAbsolutePath(), inputFileName + "-" + UUID.randomUUID().hashCode());
	...
	public void getBorderInstances(File inputPath, File outputPath, String...predNames){
		...
		try {
			A4CommandExecuter.getInstance().executeHola(reporter, this.tmpPath.getAbsolutePath(), onBorderFile.getAbsolutePath());
		} catch (Err e) {
			success = false;
			e.printStackTrace();
			logger.severe("[" + Thread.currentThread().getName() + "] " + " Failed to execute alloy* on file: " + inputPath);
            if (Configuration.IsInDeubbungMode) {
                logger.log(Level.SEVERE, "[" + Thread.currentThread().getName() + "] " + e.getMessage(), e);
            }
		}
		
		// Write generated A4Solution to outputPath
		if(success) {
			if(reporter.getA4Solution().isPresent()) {
				...
				try {
					...
					e.printStackTrace();
					logger.severe("[" + Thread.currentThread().getName() + "] " + " Failed to write alloy* solution to XML file: " + outputPath);
		            if (Configuration.IsInDeubbungMode) {
		                logger.log(Level.SEVERE, "[" + Thread.currentThread().getName() + "] " + e.getMessage(), e);
		            }
				}
