 ...
 */
pred crossRiver1 [from, from', to, to': set Object] {
 ...
 */
pred stateTransition1 {
  all s: State, s': ord/next[s] {
    Farmer in s.near =>
      crossRiver1[s.near, s'.near, s.far, s'.far] else
      crossRiver1[s.far, s'.far, s.near, s'.near]
  }
}

pred crossRiver2 [from, from', to, to': set Object] {
  // either the Farmer takes no items
  ( from' = from - Farmer &&
...
pred stateTransition2 {
  all s: State, s': ord/next[s] {
    Farmer in s.near =>
      crossRiver2[s.near, s'.near, s.far, s'.far] else
      crossRiver2[s.far, s'.far, s.near, s'.near]
  }
}
...
run  {! (solvePuzzle1<=> solvePuzzle2 )} for 8 State
run  {! (solvePuzzle1 => solvePuzzle2 )} for 8 State
run  {! (solvePuzzle2 => solvePuzzle1 )} for 8 State
run  { (solvePuzzle1<=> solvePuzzle2 )} for 8 State
run  { (solvePuzzle1 => solvePuzzle2 )} for 8 State
run  { (solvePuzzle2 => solvePuzzle1 )} for 8 State
